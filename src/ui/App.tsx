...
const initial:Config={enabled:false,output:'0',layer:null,elementId:'',preset:'black-soft',mode:'box',color:'#000000',opacity:.55,paddingX:24,paddingY:12,lineGap:0,radius:10,borderEnabled:false,borderColor:'#ffffff',borderOpacity:.5,borderWidth:1,stripeWidth:90};
...
export function App(){
  ...
  useEffect(()=>{const off=onMessage(raw=>{const m=raw as State;if(m?.type==='state')setState(m)});postMessage({type:'init'});return off},[]);
  const set=(patch:Partial<Config>)=>postMessage({type:'config',config:{...state.config,...patch,preset:'custom'}});
  ...
  return <Root>
    ...
    <Panel label="Predefinição"><Select value={c.preset} onChange={e=>postMessage({type:'preset',preset:e.target.value})}>
